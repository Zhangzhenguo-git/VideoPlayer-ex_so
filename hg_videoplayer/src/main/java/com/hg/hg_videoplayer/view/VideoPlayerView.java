package com.hg.hg_videoplayer.view;
...
import com.hg.hg_videoplayer.vide_interface.PlayInterface;
...
public class VideoPlayerView extends FrameLayout implements BaseVideoPlayerView.VideoViewTouchListening ,GSYVideoProgressListener,  VideoAllCallBack {

    private BaseVideoPlayerView video_player;
    ...
    public  void init() {
        LayoutInflater.from(getContext()).inflate(R.layout.videoplayview, this);
        ...
        mediaPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "max-buffer-size", 100000000);
...
//        proxyCacheManager=ProxyCacheManager.instance();
//        systemPlayerManager=new SystemPlayerManager();


//        ijkMediaPlayer.setOnErrorListener(new GSYVideoBaseManager() {
//            @Override
//            public boolean onError(IMediaPlayer mp, int what, int extra) {
//                if (what==-10000){
//                    Toast.makeText(context, "失败0", Toast.LENGTH_SHORT).show();
//                    return true;
//                }else {
...
//        });
     ...
     * @param url 播放地址
     */
    public void setVideoURL(String url,String title,boolean cacheWithPlay) {
//        带缓存和不带缓存路径都试了，都不行
//        File filePath= Environment.getExternalStorageDirectory();
        if (video_player != null) {
//            video_player.setUp(url,true,filePath,title);
            video_player.setUp(url,cacheWithPlay,title);
        }
    }
    ...
    public void onVideoResume(){
        if (video_player != null) {
//            video_player.getCurrentPlayer().onVideoResume(true);
            video_player.onVideoResume(true);
        }
    }
     ...
     * @return 视频对象
     */
    public BaseVideoPlayerView getVideo_player() {
        return video_player;
    }
